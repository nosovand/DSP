...
% odhad spektra sumu
%%%%%%%%%%%%%%%%%%%%%%%
Nn = myAbsSpecEstimate(noise,wlen,wstep);
...
for i=1:wnum,
 ...
 % yframe = frame ;
  ...
  % odstraneni sumu odectenim N spektra viz. Wieneruv filtr cviko
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  F = fft(frame,wlen);
  Fabs = abs(F);
  Fclean = Fabs - Nn;
  Fclean = max(Fclean,0); %odstraneni zapornych hodnot
  Fclean_complex = Fclean.*exp(1i*angle(F));
  yframe = real(ifft(Fclean_complex,wlen));
